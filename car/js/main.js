...
function createSpriteShape() {
    ...
    //数据源，要在画布上绘制图像的起始X坐标，要在画布上绘制图像的起始y坐标。
...
//创建一个动画循环，在每一帧更新动画混合器，并执行渲染操作
function animate() {
    //requestAnimationFrame这个函数就相当循环调用,,,就是说，每一帧都会调用animate这个函数，一直循环调用animate，循环调用render函数
...
THREE.OrbitControls = function ( object, domElement ) {
    ...
    function onMouseDown( event ) {
        ...
        console.log( 'onMouseDown' ,'event = '+event.button);
        console.log( 'onMouseDown' ,'event = '+scope.mouseButtons.ORBIT);
        ...
        switch ( event.button ) {
            //这里打印的event.button只有scope.mouseButtons.ORBIT类型的，不知道其他两个类型是用于什么
    ...
    //监听键盘按下事件，当按下键盘的时候就调用onKeyDown方法
...
function init() {
    ...
    loader.load( fbxSrc, function( object ) {
        ...
        //action.play(); 直接调用action.play就会循环播放所有车门+后备箱的开关动画
            ...
            if ( child instanceof THREE.Mesh ) {
                ...
                dots.forEach(function(v,k){
                    if(new RegExp(v.label).test(child.name)){
                        console.log(child.name,v.label);
    ...
    console.log("min polor angle is ",controls.minPolarAngle);
    //设置一个极角度数，极角表示相机向上的最值角度，这样可以把相机的角度限制再一定范围内
    ...
    //设置不可缩放，放得太大的时候直接看到车辆内部，给人感觉模型坏掉了
    controls.enableZoom = false;
    // camera.position.set(-1.7077679027035708, -0.043401824895598545,0.9930627089978632);
    //初始化模型展示的位置
    camera.position.set(-1.765107923988093, 0.3935498455700958,0.7962669198435924);
    controls.update();
    //监听窗口变化，当窗口变化的时候调用onWindowResize方法重新渲染，使车辆模型处于居中显示的状态
    ...
    function onMouseDown( event ) {
        ...
        //也就是当前鼠标点击的时候的坐标位置，范围是-1到1
        mouse.x = ( event.clientX / window.innerWidth ) * 2 - 1;
        mouse.y = - ( event.clientY / window.innerHeight ) * 2 + 1;
        console.log("onMouseDown mouse.x = ",mouse.x);
        console.log("onMouseDown mouse.y = ",mouse.y);
        // 通过摄像机和鼠标位置更新射线
        ...
        group.traverse(function(child){
            if ( child instanceof THREE.Sprite ) {
                ...
                console.log("onMouseDown intersects is ",intersects);
	...
	object: Cc {
		...
		frustumCulled: true
		layers: Pd {mask: 1}
		material: Za {uuid: 'E1698BEE-3081-4006-9888-6B21958FA7AC',name: '',type: 'SpriteMaterial',fog: false,lights: false,…}
		matrix: K {elements: Array(16)}
		matrixAutoUpdate: true
		matrixWorld: K {elements: Array(16)}
		matrixWorldNeedsUpdate: false
		name: "fl"
		open: 0
		openDur: 2400
		parent: pa {uuid: '2C8BC1E6-2E57-4497-A888-9417939ABB32',name: 'Door_FL03',type: 'Mesh',parent: Fc,children: Array(1),…}
		position: p {x: 0,y: 110,z: 100}
		quaternion: Z {_x: 0,_y: 0,_z: 0,_w: 1,onChangeCallback: ƒ}
		receiveShadow: false
		renderOrder: 0
		rotation: Ya {_x: 0,_y: 0,_z: 0,_order: 'XYZ',onChangeCallback: ƒ}
		scale: p {x: 40,y: 40,z: 40}
		status: "open"
		type: "Sprite"
		up: p {x: 0,y: 1,z: 0}
		userData: {}
		uuid: "67DB43A5-3D6A-46F2-A3B2-F73900152F28"
		visible: true
		z: 1.4376118415009504
		id: 161
		modelViewMatrix: K {elements: Array(16)}
		normalMatrix: ra {elements: Array(9)}
		eulerOrder:(...)
		useQuaternion:(...)
	} */
                    var object = intersects[i].object;
                    console.log("onMouseDown object data is ",object.toString);
                    var status = object.status;
                    console.log("onMouseDown object.status is ",object.status);
                    //点击的时候获取到当前状态是关闭的话就需要把门打开
                    if(status  === 'close'){
                        //将动画时间设置为open的值？？
                        action.time = object.open;
                        console.log("onMouseDown object.open = ",object.open);
                        action.paused = false;
                        //播放动画
                        action.play();
                        //更新状态为'open'
                        ...
                        //开门的时候需要把其他门状态都置为关闭
                        closeOther(object);
                        //设置一个定时器在object.openDur时间后停止动画
                        timer = setTimeout(function(){
                            action.paused = true;
                            if(object.name == 'fl' || object.name == 'fr'){
                                //好吧，这个方法是定义在index.html里面的，只做了一句打印，没啥用
                                openDoor();
                            }
                        },object.openDur);
                    }else{
                        //点击的时候获取到当前状态是打开的话就需要把门关闭
    ...
    //监听鼠标按下事件，当鼠标按下的时候，就调用onMouseDown方法
    //第三个参数（可选）：捕获或冒泡（Capture or Bubble）。这是一个布尔值，用于指定事件是在捕获阶段还是冒泡阶段触发。默认情况下，事件在冒泡阶段触发（即从目标元素向上冒泡到父元素
